# OCLP-Plus Privileged Helper Tool

`com.dortania.opencore-legacy-patcher.privileged-helper` is OCLP-Plus's Privileged Helper Tool.

The architecture is as such:
1. The main application (OCLP-Plus.app) will send arguments to the privileged helper tool to execute.
...
Since running OCLP-Plus from source will lack Dortania's code signature, you will need to disable code signature verification in the privileged helper tool otherwise root commands will fail.
...
* Additionally you will be required to compile OCLP-Plus.app with your own Developer ID Application certificate

If this is not possible, we recommend using [OCLP-Plus's prebuilt binaries](../../SOURCE.md) instead.